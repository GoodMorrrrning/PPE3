...
class MissionDetail extends Controller
{
    ...
    public function DeclareFrais($idMission, Request $request)
    {
        ...
        WHERE        (dbo.MISSION.ID_MISSION = ?) AND (dbo.Type_Frais.NomType = ?) AND (Type_Frais_1.NomType = ?) AND (Type_Frais_2.NomType = ?)', array($idMission, 'hotel', 'carburant', 'manger'));
        // $url = storage::url($ancienImg)
        //----Get urls-----------------------------//


        // on regarde si le input contien un fichier
        if($request->hasFile('Ticketimg')==true){
        ...
        //sinon on re enregistre celui qu'il a déja
            ...
            foreach($ImgData as $bclTicketHotel){
                $url = $bclTicketHotel->TicketHotel;
            }
        }
        if($request->hasFile('ticketcarbu')==true){
            ...
            foreach($ImgData as $bclTicketcarbu){
                $urlCarbu = $bclTicketcarbu->TicketCarbu;
            }
        }
        if($request->hasFile('ticketmanger')==true){
            ...
            foreach($ImgData as $bclTicketmanger){
                $urlManger = $bclTicketmanger->TicketManger;
            }
        }
